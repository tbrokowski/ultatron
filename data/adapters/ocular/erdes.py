...
class ERDESAdapter(BaseAdapter):
    ...
    def _resolve_dataset_root(cls, root: str | Path) -> Path:
        root = Path(root)
        if (root / "clips").is_dir() and (root / "splits").is_dir():
            return root
        ...
        if (candidate / "erdes_metadata.csv").exists():
            return candidate
        raise FileNotFoundError(f"{cls.DATASET_ID}: expected clips/ and splits/ under {root}")
    ...
    def iter_entries(self) -> Iterator[USManifestEntry]:
        ...
        if metadata_path.exists():
            yield from self._iter_from_metadata(metadata_path)
            return
        yield from self._iter_from_splits()

    def _iter_from_metadata(self, metadata_path: Path) -> Iterator[USManifestEntry]:
    ...
    def _iter_from_splits(self) -> Iterator[USManifestEntry]:
        """Build entries from splits/{variant}/{train,val,test}.csv when metadata CSV is absent."""
        split_dir = self.root / "splits" / self.split_variant
        ...
        for split_name in ("train", "val", "test"):
            ...
            with csv_path.open(newline="") as f:
                for row in csv.DictReader(f):
                    rel_path = (row.get("path") or row.get("file_path") or "").strip()
                    if rel_path:
                        rows.append((rel_path, row.get("label", "0"), split_name))

        for idx, (rel_path, label_raw, default_split) in enumerate(rows):
            vpath = self.root / "clips" / rel_path
            if not vpath.exists():
                continue
            ...
            study_id = clip_id.rsplit("_", 1)[0]
            split = self.split_override or default_split
            cls_label = int(label_raw) if str(label_raw).isdigit() else 0
            diagnostic_class = "rd" if cls_label else "non_rd"
            label_ontology = "retinal_detachment" if cls_label else "retina"
