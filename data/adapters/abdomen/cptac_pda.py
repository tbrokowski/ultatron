"""
data/adapters/abdomen/cptac_pda.py  - CPTAC-PDA ultrasound DICOM adapter
========================================================================

CPTAC-PDA (TCIA): pancreatic ductal adenocarcinoma radiology collection.
This adapter indexes the ultrasound (US) DICOM series staged under dicoms/.
...
class CPTACPDAAdapter(BaseAdapter):
    DATASET_ID     = "cptac-pda"
    ANATOMY_FAMILY = "abdominal"
    SONODQS        = "gold"
    DOI            = "https://doi.org/10.7937/k9/tcia.2018.sc20fo18"
    ...
    def _ensure_extracted(dicoms_dir: Path) -> None:
        ...
        for zip_path in sorted(dicoms_dir.glob("*.zip")):
            ...
            log.info("CPTACPDAAdapter: extracted %s", zip_path.name)
    ...
    def iter_entries(self) -> Iterator[USManifestEntry]:
        if not self._dicoms_dir.is_dir():
            log.warning("CPTACPDAAdapter: dicoms/ not found under %s", self.root)
            return
        ...
        if not series:
            if any(self._dicoms_dir.glob("*.zip")):
                log.warning(
                    "CPTACPDAAdapter: DICOM zips present but no extracted "
                    "series found under %s",
                    self._dicoms_dir,
                )
            else:
                log.warning(
                    "CPTACPDAAdapter: no DICOM series found under %s",
                    self._dicoms_dir,
                )
            return
