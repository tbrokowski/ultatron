...
# from dataset.dataset_usmsk import CropDataset
...
def eval_linear(args):
    ...
    # dataset_val = CropDataset(csv_file=knn_val_csv, root_dir=knn_val_root, transform=val_transform)
    if args.dataset == 'fetal_planes':
        dataset_val = FetalPlanesDataset(data_root=args.data_path, split='test', transform=val_transform)
    elif args.dataset == 'busi':
        dataset_val = BUSIDataset(data_root=args.data_path, split='validation', transform=val_transform)
    ...
    # dataset_train = CropDataset(csv_file=knn_train_csv, root_dir=knn_train_root, transform=train_transform)
    if args.dataset == 'fetal_planes':
        dataset_train = FetalPlanesDataset(data_root=args.data_path, split='train', transform=train_transform)
    elif args.dataset == 'busi':
        dataset_train = BUSIDataset(data_root=args.data_path, split='training', transform=train_transform)
    ...
    optimizer = torch.optim.SGD(
        ...
        args.lr * (args.batch_size_per_gpu/16),
        momentum=0.9,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch % args.val_freq == 0 or epoch == args.epochs - 1:
            ...
            # Only save checkpoint if current accuracy is better than or equal to best so far
            if test_stats["acc1"] >= best_acc:
                print(f"New best accuracy: {test_stats['acc1']:.2f}% (previous best: {best_acc:.2f}%)")
                save_dict = {
                    "epoch": epoch + 1,
                ...
                torch.save(save_dict, os.path.join(args.output_dir, "checkpoint_{}_linear_{}.pth".format(args.checkpoint_key, args.ckp_name)))
            else:
                print(f"Current accuracy: {test_stats['acc1']:.2f}% (best remains: {best_acc:.2f}%)")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--arch', default='vmamba_small', type=str, choices=['vit_tiny', 'vit_small', 'vit_base', 
    ...
    parser.add_argument('--log_name', default='vmamba_small', type=str, help='Name of the log file')
    parser.add_argument('--ckp_name', default='vmamba_small', type=str, help='Name of the checkpoint file')
