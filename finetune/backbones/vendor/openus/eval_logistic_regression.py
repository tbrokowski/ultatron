...
from datasets_usmsk import CropDataset
from vmamba_models.dino_vmamba import Backbone_DINOv2_VSSM, Backbone_DINOv2_VSSM_2
...
def extract_feature_pipeline(args):
    # ============ preparing data ... ============
    knn_train_csv = os.path.join(args.data_path, "train", "train_crops.csv")
    knn_val_csv = os.path.join(args.data_path, "val", "val_crops.csv")
    knn_train_root = os.path.join(args.data_path, "train")
    knn_val_root = os.path.join(args.data_path, "val")
    ...
    dataset_val = CropDataset(csv_file=knn_val_csv, root_dir=knn_val_root, transform=val_transform, return_idx=True)
    ...
    dataset_train = CropDataset(csv_file=knn_train_csv, root_dir=knn_train_root, transform=train_transform, return_idx=True)
    ...
    if 'swin' in args.arch:
        ...
        model = Backbone_DINOv2_VSSM(pretrained='./pretrained/vmamba/vssm_small_0229_ckpt_epoch_222.pth')
    else:
        model = models.__dict__[args.arch](
            patch_size=args.patch_size, 
            num_classes=0,
    ...
    if args.arch == 'vmamba_small':
        if os.path.isfile(args.pretrained_weights):
            ...
            if args.checkpoint_key is not None and args.checkpoint_key in checkpoint:
                ...
                # Simplify the loading process - create a clean state dict for the model
                clean_state_dict = {}
                for k, v in state_dict.items():
                    # Remove the 'backbone.' prefix from keys
                    if k.startswith('backbone.'):
                        new_key = k[9:]  # Remove 'backbone.'
                        clean_state_dict[new_key] = v
                
                # Try direct loading with non-strict mode
                try:
...
def logistic_regression(train_features, train_labels, test_features, test_labels, lambd, args):
    ...
    classifier = Classifier(loss="multiclass-logistic", 
                        penalty="l2", 
                        fit_intercept=False, 
                        lambda_1=lambd, 
                        lambda_2=lambd, 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--arch', default='vmamba_small', type=str, choices=['vit_tiny', 'vit_small', 'vit_base', 
        'vit_large', 'swin_tiny','swin_small', 'swin_base', 'swin_large', 'resnet50', 'resnet101', 'vmamba_small'], help='Architecture.')
