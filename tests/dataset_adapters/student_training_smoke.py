"""
tests/dataset_adapters/student_training_smoke.py
=================================================
Full 800-step smoke run for the single-student (Hiera) pretraining pipeline.

Loads real pretrained models (Hiera student, DINOv3-L, V-JEPA2-L) and trains
...
Usage (inside the EDF container):
    ...
    python -m tests.dataset_adapters.student_training_smoke
    python -m tests.dataset_adapters.student_training_smoke --resume

Multi-GPU (default via scripts/submit_student_smoke.sh):

    torchrun --nproc_per_node=4 -m tests.dataset_adapters.student_training_smoke
...
---------------------
    ...
    US_SMOKE_STEPS               Override total_steps (default 800)
    US_STUDENT_CONFIG            Path to YAML (default smoke; pretrain uses configs/student/student_pretrain.yaml)
    US_STUDENT_LR                Override training.lr
    US_STUDENT_WARMUP            Override training.warmup_steps
    US_STUDENT_CKPT_DIR          Override checkpoint directory
"""
...
from data.pipeline.student_datamodule import StudentDataConfig, StudentDataModule, StudentMixedCollator
...
def _setup_logging() -> logging.Logger:
    ...
    # stderr — captured in Slurm .err
    err_handler = logging.StreamHandler(sys.stderr)
    err_handler.setFormatter(fmt)
    logger.addHandler(err_handler)
    # stdout — captured in Slurm .out (rank 0 milestones only via _announce)
    out_handler = logging.StreamHandler(sys.stdout)
    out_handler.setFormatter(fmt)
    out_handler.addFilter(lambda r: _is_main())
...
class StudentSmokeTrainer:
    ...
    def save_checkpoint(self, step: int, stage: int, path: Path) -> None:
        if not _is_main():
            return
...
def run_training(
    ...
    if _is_main():
        ...
        _announce(
            ...
            f"DDP={_is_ddp()} | grad_accum={grad_accum} | ckpt_dir={trainer.ckpt_dir}"
        )
        log.info(
            "Starting %d-step %s run%s | stages: %s | DDP=%s | grad_accum=%d | ckpt_dir=%s",
            total, run_kind, resume_note, stage_bounds, _is_ddp(), grad_accum, trainer.ckpt_dir,
        )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Student pipeline smoke / pretrain run")
    parser.add_argument(
        "--resume",
        action="store_true",
